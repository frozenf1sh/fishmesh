...
const (
	...
	exactStatusAvailable = "available"
...
func (c *Client) do(ctx context.Context, body []byte, prefixKey string) (*http.Response, error) {
	...
	if prefixKey != "" {
		request.Header.Set(headerPrefixKey, prefixKey)
	}
...
func decisionHeaders(headers http.Header) DecisionHeaders {
	return DecisionHeaders{
		...
		ExactStatus: headers.Get(HeaderExactStatus), CachedPrefixTokens: cachedPrefixTokens(headers.Get(HeaderCachedPrefixTokens)), Upstream: headers.Get(HeaderUpstream),
	}
}
...
func (h DecisionHeaders) String() string {
	return fmt.Sprintf("routing_mode=%s route_reason=%s policy=%s exact_status=%s cached_prefix_tokens=%d backend_id=%s preferred_backend_id=%s upstream=%s spillover_reason=%s",
		h.RoutingMode, h.RouteReason, h.Policy, h.ExactStatus, h.CachedPrefixTokens, h.BackendID, h.PreferredBackendID, h.Upstream, h.SpilloverReason)
}
