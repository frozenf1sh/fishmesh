...
func TestRoutingInputFromMatchesPreservesValidZeroAndUnknown(t *testing.T) {
	...
	if input.Matches["a"] != (routing.CacheMatch{Valid: true, MatchedTokens: 0}) {
...
func TestExactPathBuildsKVQueryFromReadonlyTokenIDsAndDegradesUnknown(t *testing.T) {
	...
	path, err := New(Config{Service: backend.Backend{ID: "service", URL: "http://service:8000"}}, Dependencies{
		Resolver: resolver, Strategy: routing.NewExactCacheLoad(), Circuits: newTestBreaker(t), Tokenizer: tokenizer, KVCache: cache,
	...
	if lease.Decision.Reason != routing.ReasonExactSignalUnavailable || lease.State.Exact != ExactMatchUnavailable {
		t.Fatalf("unknown cache decision/state = %+v / %q", lease.Decision, lease.State.Exact)
	}
}

func TestExactPathPreservesCanceledTokenization(t *testing.T) {
...
func TestExactPathRequiresTokenizerAndKVCache(t *testing.T) {
	resolver := &mutableResolver{}
	_, err := New(Config{Service: backend.Backend{ID: "service", URL: "http://service:8000"}}, Dependencies{
		Resolver: resolver, Strategy: routing.NewExactCacheLoad(), Circuits: newTestBreaker(t),
	})
	if err == nil {
		t.Fatal("exact path accepted missing tokenizer and KV cache")
	}
}
