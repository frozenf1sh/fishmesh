...
2. session-key 是否只在热点前缀下有效；
3. 简单的 Gateway In-flight load-balanced 策略是否能改善混合负载尾延迟；
...
| legacy key-hash + keep-alive | 51.92 ms | 79.83 ms | 51.34 ms | 200/200 |

结论：相比 no keep-alive，Generic Keepalive 将 P50 降低约 43%，是当前最确定的收益。legacy key-hash 的 P95 低约 14%，但 P50 和 Warm P50 略差，不能据此宣称 KV Cache 命中提升。
...
| legacy key-hash + keep-alive（rerun） | **42.68 ms** | **62.64 ms** | **83.43 ms** | 200/200 |
| In-flight load-balanced | 52.51 ms | 65.94 ms | 89.01 ms | 200/200 |

第一次 hot-prefix session-key attempt 为 196/200 成功，P50/P95/P99 分别为
...
| legacy key-hash + keep-alive | **46.10 ms** | 105.17 ms | **584.41 ms** | 200/200 |
| In-flight load-balanced | 49.78 ms | **94.70 ms** | **140.17 ms** | 200/200 |

纯 legacy key-hash 在混合负载下没有改善 P95，P99 反而显著变差；In-flight load-balanced 的 P95/P99 更稳定，但 P50 略差。说明生产策略不能只做 `hash(prefix) -> pod`，必须把局部性和当前负载共同纳入决策。
...
| In-flight load-balanced probe | 58.18 ms | 220.47 ms | 94/100 | 没有外部 GPU/队列信号，仍可能选中饱和 endpoint |

这组不是严格的 A/B 性能结论，因为两次压力窗口和 endpoint 状态不完全相同；它的工程价值是证明“Gateway 自己的 in-flight 计数不等于 vLLM/GPU 负载观测”，真正的 load-balanced 必须接入 vLLM metrics、队列和 GPU 信号。
...
- 每个条件的重复次数和模型规模仍有限；
- legacy key-hash 使用的是 Pod IP 快照，故障恢复实验已证明其工程风险。
...
2. **session-key 是条件策略，不是所有负载下的默认策略。** 热点前缀明显受益，混合/热点不均时可能制造 tail latency。
3. **简单 In-flight load-balanced 有研究价值，但还不是 GPU-aware Scheduler。** 它在混合负载中改善了尾延迟，却无法感知外部压力和 vLLM 队列。
4. **下一步应验证 session-key：硬 eligibility、局部性优先和过载 spillover，而不是直接实现多指标加权 score。**
