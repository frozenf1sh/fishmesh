...
本阶段只实现 cooperative session-key 有界粘性的安全快路径，不宣称读取 vLLM 的真实 KV
...
| 环境变量 | 默认值 | 含义 |
| --- | --- | --- |
| `FISHMESH_SESSION_KEY_TTL` | `5m` | 活跃 routing key 的滑动过期时间 |
| `FISHMESH_SESSION_KEY_MAX_ENTRIES` | `10000` | registry 硬上限 |
| `FISHMESH_SESSION_KEY_INFLIGHT_DELTA` | `2` | local in-flight 相对差值 |
| `FISHMESH_SESSION_KEY_QUEUE_DEPTH_DELTA` | `1` | vLLM queue depth 相对差值 |
...
归档：`artifacts/published/2026-08-09-session-key-runtime-smoke-attempt-2/`。
...
- 决策：1 次 session-key miss、11 次 hit、12 次 local-inflight spillover；两个 backend
...
集群已恢复默认 `inflight_delta=2`、`queue_depth_delta=1` 的 session-key overlay。
...
这些属于后续 P1/P2 条目，不能因为 session-key-v1 已能路由就宣称 MVP 性能结论成立。
