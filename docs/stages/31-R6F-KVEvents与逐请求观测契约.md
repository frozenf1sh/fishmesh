...
4. `available` 的 KV-aware 选择才记录一次最终 backend 的 `cached_prefix_tokens` histogram；其值为零时
   是真实零命中，必须保留。`match-unavailable`、tokenization/lookup failed 和未请求 KV-aware 不记录
   cached-prefix 零样本。
...
- Gateway `Metrics` 将 callback 投影为 `fishmesh_gateway_kv_event_publish_to_apply_seconds` histogram，
  并将可用 KV-aware 选择投影为 `fishmesh_gateway_kv_aware_cached_prefix_tokens` histogram。
...
3. 在实际 KV-aware Lite overlay 上以两条 SSE 请求进行低负载验证，检查 Prometheus histogram 和
   dashboard 查询；不运行长时或并行 GPU 压测；
4. 结束后恢复 `session-key` 基线，执行 `go test -race ./...`、`go vet ./...`、`go build ./...`、
...
KV-aware overlay 已有的 KVEvents/replay 配置保持不变。启动日志确认 `routing_mode=kv-aware`。最终
`r6f-r2` 仅包含 backend label 回收并作为恢复后的 session-key Gateway 镜像部署。
以同一个约 3.8KiB system prompt、不同 user message、无 session key 发送两条短 SSE 请求：

| 请求 | KV-aware status / cached prefix tokens | 结论 |
| --- | --- | --- |
...
Prometheus 原始 `fishmesh_gateway_kv_aware_cached_prefix_tokens_count/sum` 为 `2/768`。新的 live
...
本次仅两条短请求，不是 benchmark；验收结束后恢复 `deploy/experiments/r6d-session-key`。
