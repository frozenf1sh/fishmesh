...
| 32 | R6G Go 对话与压测客户端 | 独立 chat/request/bench、隐私安全 history/JSONL、终端色彩诊断与 bounded-affinity 真实短验收 |
| 33 | R6H 成本式 Exact 路由 | 以等价未缓存 token 合并 cache/queue/running/local in-flight；GPU 集群验收后置 |
