# 阶段 33：R6H 成本式 Exact 路由

> 状态：本地契约与实现中；GPU 节点关闭期间不部署、不访问 Kubernetes、不做性能结论。

## 1. 用更简单的话说

现在的 exact 会先问“谁已经有更多缓存”，只有缓存数量完全相同时才问“谁更忙”。所以一次冷请求在
...
1. `routing` 继续是纯函数域，只消费 `ExactInput`、`Load`、local in-flight 与 routing-owned
   `ExactCacheLoadConfig`；不 import tokenization、kvcache、HTTP、Kubernetes 或 Prometheus。
...
3. `Load.Valid=false` 不等于 queue/running 为零：只省略这两个未知项；local in-flight 仍是 Gateway 已知
   的本地事实。unknown/stale KV match 仍由 requestpath 降级，不能进入 exact 成本计算。
4. hard overload 仍先排除，所有候选都 hard-overload 时保持现有 load-only fallback；成本模型不能绕过
...
1. 在 `routing` 定义成本配置和值对象，先以同包 contract tests 锁定 cache/load 取舍、unknown、硬过载、
   平局与整数边界。
2. 将 exact 策略从词典比较替换为饱和成本比较，并把 policy 标为 `exact-cache-load-v2`；保留 v1 的
   load-aware degradation policy，避免把 KV unknown 变成 zero match。
...
4. GPU 恢复后才启用新镜像/配置，并按顺序验收：两 Pod KV valid → cold equal-cost → shared-prefix hit →
   人为制造 queue/running 压力 → stale/replay 降级 → bounded-affinity 恢复。每项保留 headers、metrics、
...
- 若真实数据表明 queue/running 指标不可靠，保留有效 KV 与 local in-flight，但不伪造外部 load；必要时
  回到 `exact-load-fallback-v1` 或 bounded-affinity，而不是调大 cache 权重。

## 5. GPU 恢复后的验收门槛

先确认 2/2 Ready、GPU watchdog 未告警和 exact KV replay valid。以默认并发 4、单一共享前缀和保留失败
