...
Lite MVP 需要让运行者能看见 Gateway、KVEvents 和 KV-aware 降级的健康状态，并能以可复核的
...
- `docs/notes/runbook.md`：Gateway 不可用、KV 信号无效/replay 过期、KV-aware 降级和 GPU 温度
  事件的取证与恢复步骤；
...
| `FishMeshKVAwareSignalUnavailable` | 所有实例 `kv_cache_instance_valid` 连续 2 分钟为 0 | 取证 subscriber/replay/EndpointSlice，保留 load-balanced 降级 |
| `FishMeshKVReplayStale` | freshness 大于 30 秒超过 2 分钟 | 检查 KVEvents/replay 链路和 sequence 日志 |
| `FishMeshKVAwareDegradationRateHigh` | 5 分钟降级速率高于 0.1/s 持续 10 分钟 | 按 reason 区分 stale/unknown/overload，禁止把 unknown 当 miss |
