...
1. 默认部署使用 `load-balanced`；`session-key` 通过独立 overlay 显式开启，KV-aware 只由
   `deploy/lite-kv-aware` 显式开启，演示结束必须恢复 baseline。
2. KV-aware 演示使用两个不带 `X-FishMesh-Session-Key` 的请求，共用一段足以跨多个 KV block 的 system
   prompt、使用不同 user message，并检查决策头而不是推断缓存状态。
3. `match-unavailable`/`kv-aware-load-fallback-v1` 是 replay 未确认时的正确 load-balanced 降级；
   `available` 且 cached tokens 为 0 才是可参与 KV-aware 的真实零命中。
4. R6E Standard mode/llm-d 部署后置，不把已有 adapter 当成已经完成的产品安装面。

## 实际完成度

R6A–R6C 的 Lite KV-aware 数据面和 R6D/D2 的有限 profile 已完成；README 的路线图相应收口。
