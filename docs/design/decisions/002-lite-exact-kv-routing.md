...
`fishmesh-epp` 和一个 bounded-affinity scorer 作为唯一生产交付物，会出现三个问题：
...
4. **复用上游 KV library，不复制 engine 细节。** Lite mode 优先通过薄 adapter 复用固定版本
   ...
   protocol 或分词器。
5. **客户端会话键降为可选提示。** 当前 `X-FishMesh-Prefix-Key` 在兼容期内保留，但其语义
   明确为 session affinity。它只用于 exact 数据未就绪、平局或短期稳定性，不再作为缓存命中
   的事实来源。
...
OpenAI client
  -> fishmesh-gateway Service / Deployment
       ...
       -> local exact KV block index from vLLM KVEvents
...
- exact cache sample 新鲜：使用 cache + load 联合选择；
- cache sample 缺失、过期或事件流不可信：使用 load-aware；
...
- subscriber 不健康时 readiness/metric 能说明 exact 能力降级；
...
5. event stream 过期时路由输入明确标记 invalid，并能降级到 load-aware；
...
- ADR-001 中“standalone 只保留开发/演示”和“精确 KV cache 不进入项目”的范围被本 ADR
  替代；
- bounded-affinity-v1 保留为兼容与降级策略，不再代表最终主能力；
- R5D 标准部署不取消，但顺序后移到 Lite exact KV MVP 和产品化之后；
